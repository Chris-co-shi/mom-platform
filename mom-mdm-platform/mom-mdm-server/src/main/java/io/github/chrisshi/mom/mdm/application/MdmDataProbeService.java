...
public class MdmDataProbeService {
     ...
     * 表示记录不存在或已经被其他事务修改，调用方不得把它当作成功覆盖。</p>
     *
     * @param id 数据库主键
     ...
     */
    @Transactional
    public boolean updateValue(Long id, Long expectedVersion, String probeValue) {
        if (id == null || id <= 0) {
            throw new IllegalArgumentException("id 必须大于零");
        }
