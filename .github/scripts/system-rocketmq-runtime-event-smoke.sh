...
ROCKETMQ_NETWORK="mom-system-runtime-network-${$}-${RANDOM}"
...
ROCKETMQ_HOST_NAME_SERVER="127.0.0.1:9876"
ROCKETMQ_CONTAINER_NAME_SERVER="${NAMESRV_CONTAINER}:9876"
...
CACHE_KEY_PREFIX="mom:${ENVIRONMENT}:_global:system:cache:v1:parameter-resolved"
...
cleanup() {
  ...
  docker network rm "$ROCKETMQ_NETWORK" >/dev/null 2>&1
  rm -f "$BROKER_CONFIG"
  rm -rf "$KEY_DIR"
...
wait_broker() {
  for attempt in {1..90}; do
    if docker exec "$BROKER_CONTAINER" sh mqadmin clusterList \
        -n "$ROCKETMQ_CONTAINER_NAME_SERVER" 2>/dev/null | grep --quiet 'DefaultCluster'; then
      return 0
    fi
    sleep 2
  done
...
cache_key() {
  local version="$1"
  printf '%s:_global:%s:global:_global:%s' "$CACHE_KEY_PREFIX" "$PARAMETER_KEY" "$version"
}
...
assert_cache_present() {
  ...
  actual=$(docker exec "$REDIS_CONTAINER" redis-cli EXISTS "$key" | tr -d '[:space:]')
  [[ "$actual" == "1" ]] || fail "Parameter Runtime typed Cache Key was not populated for version ${version}"
}
...
assert_cache_absent() {
  ...
  local actual
  key=$(cache_key "$version")
  actual=$(docker exec "$REDIS_CONTAINER" redis-cli EXISTS "$key" 2>/dev/null | tr -d '[:space:]')
  [[ "$actual" == "0" ]] || fail "Unexpected Parameter Runtime typed Cache Key for version ${version}"
}
...
docker network create "$ROCKETMQ_NETWORK" >/dev/null

docker run --name "$NAMESRV_CONTAINER" --network "$ROCKETMQ_NETWORK" \
  -p 127.0.0.1:9876:9876 \
  -e JAVA_OPT_EXT='-Xms256m -Xmx256m -Xmn128m' \
  -d "$ROCKETMQ_IMAGE" sh mqnamesrv >/dev/null

docker run --name "$BROKER_CONTAINER" --network "$ROCKETMQ_NETWORK" \
  -p 127.0.0.1:10909:10909 -p 127.0.0.1:10911:10911 -p 127.0.0.1:10912:10912 \
  -e NAMESRV_ADDR="$ROCKETMQ_CONTAINER_NAME_SERVER" \
  -e JAVA_OPT_EXT='-Xms384m -Xmx384m -Xmn128m' \
  -v "$BROKER_CONFIG:/home/rocketmq/broker.conf:ro" \
  -d "$ROCKETMQ_IMAGE" \
  sh mqbroker -n "$ROCKETMQ_CONTAINER_NAME_SERVER" -c /home/rocketmq/broker.conf >/dev/null
...
docker exec "$BROKER_CONTAINER" sh mqadmin updateTopic \
  -n "$ROCKETMQ_CONTAINER_NAME_SERVER" -c DefaultCluster -t "$EVENT_TOPIC" >/dev/null
...
ROCKETMQ_NAME_SERVER="$ROCKETMQ_HOST_NAME_SERVER" ROCKETMQ_MAX_RECONSUME_TIMES=2 \
...
for attempt in {1..180}; do
  DLQ_OUTPUT=$(docker exec "$BROKER_CONTAINER" sh mqadmin topicStatus \
    -n "$ROCKETMQ_CONTAINER_NAME_SERVER" -t "$DLQ_TOPIC" 2>/dev/null || true)
...
printf '%s\n' \
  'SYSTEM_RUNTIME_EVENT_SMOKE result=success jwt_audit=success http_business_write=success outbox=sent inbox=idempotent cache=version-isolated duplicate=deduplicated broker_recovery=success redis_fallback=postgresql poison=dlq'
