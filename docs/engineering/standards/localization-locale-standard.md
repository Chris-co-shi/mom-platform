...
S15-A 审计时没有真实调用方，该历史事实保留。Chris 于 2026-07-30 后续明确批准 S15-B 建设目标消费者为 mom-web、mom-mobile 和未来 System 管理端的后端能力；当前客户端尚未接入。S15-B 采用显式发布、双 Locale 完整不可变快照、默认 Locale fallback、单调版本、回滚新版本和认证 Runtime API，不引入缓存或通知，也不进入 S16 Preference。
