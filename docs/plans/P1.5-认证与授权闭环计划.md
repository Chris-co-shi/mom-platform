...
- JWT/JWK 签发基础和 10 分钟 Access Token。

尚未具备：

- S04 JWT 完整 Roles、Permissions、Factory、Party Claims 与 `/api/iam/me`；
...
### Slice 03：Authorization Server 与账号认证 — In Progress

实现 Spring Authorization Server、OIDC、Authorization Code + PKCE S256、四个 Public Client、IAM 登录页面与 CSRF、账号密码认证、临时锁定、首次修改密码、Client 与 `user_type` 访问矩阵、JDBC OAuth Store、JWK/JWT 签发基础和 10 分钟 Access Token。

本 Slice 不提前实现 S04 完整权限 Claims、`/api/iam/me`，也不提前实现 S05 自定义 Refresh Rotation、重放检测、Session 撤销或 Redis revoked sid。
