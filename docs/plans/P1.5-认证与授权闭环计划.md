...
已完成并合并：S00、S01、S02；S03 已随提前合并的阶段 PR #20 进入 main。

S04 已在当前长期阶段分支完成并通过全量 CI：

- User → Role → Permission 有效授权计算；
- Factory Scope 与外部 Party Scope；
- Access Token `roles`、`permissions`、`factory_ids`、`party_type`、`party_id`；
- Bearer `/api/iam/me`；
- Current Factory 校验；
- 对象不可见时统一 404 的服务端基础契约。

当前 S05 正在阶段分支实现：

- Web 8 小时、Mobile 12 小时绝对 Session；
- Opaque Refresh Token 与 HMAC-SHA-256 摘要；
- 单 ACTIVE Token；
- 事务行锁 Rotation；
- 无宽限窗口的重放检测；
- Session `COMPROMISED`；
...
尚未具备：Gateway 与业务服务 Resource Server、IAM 管理 API、Web/Mobile 登录闭环和安全 E2E。
...
### Slice 05：Session、Refresh Rotation 与撤销 — In Progress
