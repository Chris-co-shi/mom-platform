...
S00～S03 已进入 main；S04、S05 已在当前长期阶段分支完成并通过全量 CI。

S05 已交付：

- Web 8 小时、Mobile 12 小时绝对 Session；
- 48 字节高熵 Opaque Refresh Token 与 HMAC-SHA-256 摘要；
- 标准 Refresh Grant、单 ACTIVE Token、PostgreSQL 行锁 Rotation；
- 无宽限窗口重放检测、Session COMPROMISED、Redis revoked sid；
- Redis Fail Closed；
- Refresh 明文不进入 OAuth JDBC Store、日志、Trace 或审计。

当前 S06 正在阶段分支实现：

- Gateway JWT 签名、Issuer、Audience、有效期和 revoked sid 校验；
- Client ID 与路由隔离；
- 外部伪造身份 Header 清理；
- Bearer Token 转发；
- 业务服务 JWT 二次验证、Permission、Factory/Party Scope 与对象归属。

尚未具备：IAM 管理 API、Web/Mobile 登录运行时、管理页面与三仓库安全 E2E。
...
### Slice 06：Gateway 与 Resource Server — In Progress
