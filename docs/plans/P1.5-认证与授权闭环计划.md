...
S00～S03 已进入 main；S04、S05、S06 已在当前长期阶段分支完成并通过全量 CI。
...
- 用户、角色、Permission 只读目录；
- Factory Scope 与 Mobile Access；
- Session 管理与安全审计查询；
- OAuth Client 查看与受控启停；
- 外部主体重新绑定；
- 至少一个有效 `PLATFORM_ADMIN` 与管理员自我保护约束。

尚未具备：Web/Mobile 登录运行时、MOM Admin 管理页面、Supplier/Customer Portal 页面与三仓库安全 E2E。
...
### Slice 07：IAM 管理 API — In Progress
