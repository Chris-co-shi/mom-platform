...
- 名称：认证与授权闭环
- 当前 Slice：`S02`
- 当前状态：**S02 Completed in branch / PR Ready**
...
S02 已在任务分支完成并进入 Review：

- IAM 独立 PostgreSQL Schema、正式 Flyway、索引、约束和完整中文注释；
- 用户、Party Binding、RBAC、Factory Scope、Mobile Access 和 Client Policy；
- Spring Security Authorization Server 官方 JDBC 协议表；
- Session、Refresh Token 安全状态、安全审计；
- Entity、Enum、Mapper、Repository、领域约束和 PostgreSQL 集成测试。

尚未具备：

- 正式 Authorization Server 配置与登录页面；
- 四个 Public Client 的环境相关 Registered Client 配置；
- JWT Claims、RBAC/Scope 运行时计算和 `/api/iam/me`；
- Refresh Rotation、重放检测、Session 撤销和 revoked sid；
- IAM 管理 API 与管理页面；
...
### Slice 02：IAM 数据库与领域模型 — Completed in branch / PR Ready
...
### Slice 03：Authorization Server 与账号认证 — Pending

实现 Spring Authorization Server、OIDC、Authorization Code + PKCE S256、四个 Public Client、IAM 登录页面与 CSRF、账号密码认证、锁定和首次修改密码。

### Slice 04：RBAC、Factory Scope 与 `/api/iam/me` — Pending

实现 User → Role → Permission、Factory/Party Scope、JWT Claims、`/api/iam/me` 和 Current Factory 校验契约。
